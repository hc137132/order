    ...
    <v-sheet width="100%" height="100%">
        <v-sheet width="100%" height="80" class=" mt-5 d-flex align-self-start justify-start">
            ...
            <v-sheet width="100%" class="d-flex justify-start align-center" height="60">
                ...
                <div class="d-flex justify-start a">
                    <div class="b">
                        <v-select v-model="obj.searchselect" class="text-body-2 ml-2" :items="['主题', '金额', '用户']"
        ...
        <v-sheet min-height="400px">
            <v-table :hover="true" fixed-header>
                <thead fixed-header class="text-center">

                    <tr class="text-h6 text-center">
                        <th class="text-center">项目主题</th>
                ...
                <tbody>
                    ...
                    <tr v-for="item in obj.resdatalist" :key="item.orderid" class="text-center stylebtn"
                        ...
                        <!-- <td>{{ item.takename ? item.takename : '无' }}</td> -->
                        <td><div calss="d-flex align-center text-center"><finish :data="item"></finish></div></td>
                        <td>{{ item.payment === '0' ? '未支付' : "已支付" }}</td>
                        <td>
                            <v-btn elevation="0" @click.stop="handledelete(item.orderid)"><v-icon icon="mdi-keyboard-return"
                                    size="27" color="error"></v-icon><v-tooltip activator="parent"
                                    location="top" >退单</v-tooltip></v-btn>
                        </td>
                    </tr>
                </tbody>
            </v-table>
...
import Finish from '@/components/user/userhome/ComFinish.vue'
...
const handleicon = (icon, item) => {
    switch (item) {
        ...
        case 'finish':
            if (icon === 'No') {
                obj.resdatalist = obj.olddatalist.filter(x => x.finish === '')
            } else {
                obj.resdatalist = obj.olddatalist.filter(a => a.finish !== '')
            }
            break
    ...
    obj.resdatalist = obj.olddatalist.filter((a) => {
        switch (obj.searchselect) {
            ...
            case '金额':
                ...
                break
            case '用户':
                if (a.email || a.takename.includes(obj.searchtext)) {
                    return a
                }
                break
        }
